...
{
    public partial class ManagerUpdateUserEntry : Page
    {
        ...
        public String
          logMsgPageInfo = "",
          logMsgPageData = "",
          pageTitle = "Manager Update User Entry",
          userMessage = "";
        ...
        protected void PagesetUp(String Name, String Alias, String ManagerName, String EmployeeType, String Status, bool RefreshControl)
        {
            try
            {
                ...
                DataTable dt = CWeb.ManagerUserListHierarchy(LoggedInUser);
                if (RefreshControl)
                {
        ...
        private void PageEntriesSetup(CUserInfo SelectedUser, DateTime EntryDate)
        {
            try
            {
                ...
                ListViewEntry.DataSource = dtGetEntries.AsEnumerable().Select
                    (row => new
                    {
        ...
        protected void ListViewEntry_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
        ...
        protected void cmdSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                CLog.WebEvent("SaveEntries Start", Page.Title, LoggedInUser);
                foreach (ListViewItem row in ListViewEntry.Items)
                {
                    switch (row.ItemType)
                    {
                ...
                if (Save == 0)
                {
                    CWeb.EntrySave(LoggedInUser, txtsdate, ListViewEntry, Page.Title);
                ...
                CLog.WebEvent("SaveEntries End", Page.Title, LoggedInUser);
            }
            catch (Exception ex)
            {
